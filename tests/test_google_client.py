...
def test_background_expired_without_refresh_token_raises(tmp_path, monkeypatch):
    # The exact symptom this fix targets: Google omitted the refresh_token, so
    # the saved token expires and cannot be refreshed. In background mode this
    # must raise, never open a browser.
    g = _client(tmp_path, token_exists=True)
...
def test_background_dead_refresh_token_raises(tmp_path, monkeypatch):
    ...
    # A revoked refresh token must still not open a browser in background mode.
    def _boom(*a, **k):
        raise AssertionError("interactive flow must not run in background mode")
...
def test_interactive_fallback_after_dead_refresh_runs_flow(tmp_path, monkeypatch):
    # With allow_interactive=True, a revoked refresh token (RefreshError) must
    # fall through to a fresh browser login rather than raising.
    g = _client(tmp_path, token_exists=True)
    creds = FakeCreds(valid=False, expired=True, refresh_token="rt",
                      refresh_error=True)
    monkeypatch.setattr(
